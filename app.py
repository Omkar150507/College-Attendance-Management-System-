...
def timetable():
    ...
    week_view=request.args.get('week','').strip().lower() in ('1','true','yes','week','all')
...
def export_timetable_pdf():
    """Export either the selected day or the complete Monday-Saturday week."""
    db=get_db(); role=session.get('role')
    days=['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday']
    selected_day=request.args.get('day','').strip()
    week=request.args.get('week','').strip().lower() in ('1','true','yes','week','all')
    ...
    rows=db.execute(f"""SELECT t.*, COALESCE(t.session_type,'Lecture') session_type,
        ...
        ORDER BY CASE t.day_of_week
            ...
            t.start_time,t.lecture_no,t.id""",tuple(params)).fetchall()
    db.close()

    buffer=io.BytesIO()
    doc=SimpleDocTemplate(buffer,pagesize=landscape(A4),rightMargin=8*mm,leftMargin=8*mm,topMargin=8*mm,bottomMargin=9*mm)
    styles=getSampleStyleSheet()
    title=ParagraphStyle('TTCollegeTitle2',parent=styles['Title'],alignment=TA_CENTER,fontSize=18,leading=21,spaceAfter=1)
    sub=ParagraphStyle('TTSub2',parent=styles['Normal'],alignment=TA_CENTER,fontSize=10,leading=12,spaceAfter=1)
    meta=ParagraphStyle('TTMeta2',parent=styles['Normal'],alignment=TA_CENTER,fontSize=9,leading=11)
    cell=ParagraphStyle('TTCell2',parent=styles['Normal'],fontSize=7.1,leading=8.3,alignment=TA_LEFT)
    head=ParagraphStyle('TTHead2',parent=cell,fontName='Helvetica-Bold',alignment=TA_CENTER)
    ...
    header_text=[Paragraph(COLLEGE_NAME,title),
                 ...
                 Paragraph(f'<b>{view_text}</b> &nbsp;&nbsp; <b>Year:</b> {class_text}',meta)]
    hdr=Table([[logo,header_text]],colWidths=[27*mm,244*mm])
    ...
    for r in rows:
        ...
        vals=([r['day_of_week']] if week else []) + [r['year'] or '—',f"{r['start_time']} – {r['end_time']}",r['session_type'] or 'Lecture',r['batch'] or 'All',subject,r['teacher_name'] or '—',r['room'] or '—']
    ...
    widths=[24*mm,18*mm,31*mm,24*mm,23*mm,64*mm,49*mm,38*mm] if week else [22*mm,34*mm,27*mm,24*mm,68*mm,50*mm,42*mm]
