...
# Display times in a user-friendly 12-hour format (e.g. 13:00 -> 1:00 PM).
def _format_time_12(value):
    if value is None:
        return ""
...
def _format_time_range_12(value):
    if value is None:
        return ""
    text = str(value).strip()
    if not text:
        return ""
    # Attendance/QR records commonly store ranges as HH:MM-HH:MM.
    parts = re.split(r"\s*(?:-|–|—|to)\s*", text, maxsplit=1, flags=re.I)
    if len(parts) == 2:
        return f"{_format_time_12(parts[0])} – {_format_time_12(parts[1])}"
    return _format_time_12(text)

app.jinja_env.filters["time12"] = _format_time_12
app.jinja_env.filters["timerange12"] = _format_time_range_12
...
def export_timetable_pdf():
    ...
    for r in rows:
        ...
        values = ([r['day_of_week']] if week else []) + [
            r['year'] or '—',
            f"{_format_time_12(r['start_time'])} – {_format_time_12(r['end_time'])}",
...
def qr_attendance():
    ...
    # Show all subjects belonging to the selected academic year. The actual
    # teacher/subject permission is checked again when a QR is generated.
    subjects=db.execute("SELECT * FROM subjects WHERE year=? ORDER BY code", (selected_year,)).fetchall()
    ...
    if request.method=='POST':
        ...
        if not subject_id or not any(s['id']==subject_id for s in subjects) or not start_time or not end_time:
            flash('Select a subject for the selected year and enter start/end time.','error')
        elif not can_use_subject(db, subject_id):
            flash('You are not assigned to this subject. Ask the admin to assign the subject to your account.','error')
        elif session_type=='Practical' and batch not in ('Batch A','Batch B','Batch C','Batch D'):
            flash('Select a valid practical batch.','error')
        else:
            scheduled=tr['teacher_id'] if tr else None
